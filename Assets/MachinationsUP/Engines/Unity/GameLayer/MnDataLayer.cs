...
namespace MachinationsUP.Engines.Unity
{
  ...
  public class MnDataLayer : IGameLifecycleSubscriber, IGameObjectLifecycleSubscriber
  {
    ...
    /// <summary>
    /// GLOBAL REPOSITORY. Provides direct access to any Machinations diagram element, based on its Machinations ID.
    /// The elements here are completely isolated from Scriptable Objects or anything else related to the infrastructure of the plugin.
    /// This also means that changes to this repository WILL NOT BE AUTOMATICALLY SENT to the Diagram.
    /// </summary>
    static public Dictionary<int, ElementBase> DiagramRepository = new Dictionary<int, ElementBase>();
    ...
    static public void UpdateSourcesWithValuesFromMachinations(List<JSONObject> elementsFromBackEnd, bool updateFromDiagram = false)
    {
      ...
      foreach (JSONObject diagramElement in elementsFromBackEnd)
      {
        ...
        //Add any and all elements ever received from the back-end to the repository, accessible anywhere.
        if (!DiagramRepository.ContainsKey(diagramMapping.DiagramElementID)) DiagramRepository.Add(diagramMapping.DiagramElementID, elementBase);
        else DiagramRepository[diagramMapping.DiagramElementID].ChangeValueTo(elementBase.CurrentValue);
